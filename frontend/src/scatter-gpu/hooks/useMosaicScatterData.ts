...
export function useMosaicScatterData({
    ...
    const positionQuery = useQuery({
        queryKey: axes
            ? scatterKeys.positions(axes.obsmKey, xCol, yCol)
    ...
    const data = useMemo((): ScatterData | null => {
        ...
        if (colorMode === "categorical") {
            ...
            return {
                ...base,
                categoryIndices:
                    catData?.categoryIndices ?? new Uint8Array(positionQuery.data.numCells),
    ...
    const loading =
        positionQuery.isFetching || categoryQuery.isFetching || continuousQuery.isFetching;
    const error =
        positionQuery.error?.message ??
        categoryQuery.error?.message ??
        continuousQuery.error?.message ??
        null;
