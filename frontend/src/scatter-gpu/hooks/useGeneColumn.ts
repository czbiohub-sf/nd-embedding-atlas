...
type GeneColumnStatus = "idle" | "loading" | "ready" | "error";

interface GeneColumnState {
  status: GeneColumnStatus;
  column: string | null;
  error: string | null;
}

export interface GeneColumnResult {
...
interface PostGeneColumnResponse {
  task_id: string;
}
 ...
 */
interface UseGeneColumnOptions {
  /** Called with a status message when loading starts/ends — use to update the bottom bar. */
  onStatus?: (msg: string | null) => void;
}

export function useGeneColumn(options?: UseGeneColumnOptions): GeneColumnResult {
  const onStatusRef = useRef(options?.onStatus);
  onStatusRef.current = options?.onStatus;

  const [state, setState] = useState<GeneColumnState>({
    status: "idle",
    column: null,
    error: null,
  });
  ...
  const materialize = useCallback((gene: string, layer: string) => {
    stopPolling();
    ...
    const run = async () => {
      ...
      try {
        ...
        const data = (await res.json()) as PostGeneColumnResponse;
        taskId = data.task_id;
      } catch (err) {
