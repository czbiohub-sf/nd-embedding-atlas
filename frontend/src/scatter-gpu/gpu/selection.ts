...
const DEBUG_SELECTION =
    typeof location !== "undefined" && new URLSearchParams(location.search).has("debug-selection");
    ...
    function readbackSelectionCount() {
        ...
        stagingBuffer
            ...
            .then(() => {
                ...
                isReadingBack = false;
                console.log(
                    `[${frame}] ${count.toLocaleString()} selected (${(performance.now() - t0).toFixed(1)}ms)`,
                );
