...
import { createUniforms, createBuffers, uploadData } from "./buffers";
import { createVertexShader, createFragmentShader } from "./shaders";
...
export async function createScatterplot(
  ...
  const adaptiveScale = Math.max(0.3, Math.min(1.5,
    Math.sqrt(referenceCount / Math.max(1, data.numCells)),
  ));

  const uniforms = createUniforms(root, canvas.width / canvas.height, config?.render);
  uniforms.paramsUniform.write(
    d.vec4f(pointRadius, canvas.width / canvas.height, selectionDimFactor, adaptiveScale),
  );
  ...
  const culling = createCullingEngine(
    root,
    device,
    buffers,
    uniforms,
  ...
  const backgroundColor = config?.render?.backgroundColor ?? [0.06, 0.06, 0.1, 1] as [number, number, number, number];
  ...
  const selection = createSelectionEngine(
    ...
    uniforms,
    data.numCells,
    (count, indices) => config?.callbacks?.onSelectionChange?.(count, indices),
  );
  ...
  if (
    ...
    new URLSearchParams(location.search).has("debug-wgsl")
  ) {
  ...
  return {
    resize(width: number, height: number) {
      ...
      uniforms.paramsUniform.write(
        d.vec4f(pointRadius, gpuW / gpuH, selectionDimFactor, adaptiveScale),
      );
      interaction.resize();
    },
