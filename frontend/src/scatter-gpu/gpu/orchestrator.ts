...
export async function createScatterplot(
    ...
    const adaptiveScale = Math.max(
        0.3,
        Math.min(1.5, Math.sqrt(referenceCount / Math.max(1, data.numCells))),
    );
    ...
    const colorPackWorkgroups = Math.ceil(
        data.numCells / (preferredWorkgroupSize * COLOR_PACK_BATCH.length),
    );
        ...
        },
        {
            ...
            onPointClick: (worldX: number, worldY: number) => {
                ...
                if (bestIdx >= 0) {
                    ...
                    config?.callbacks?.onPointClick?.(
                        bestIdx,
                        [px, py],
                        catIdx,
                        data.categoryNames[catIdx],
                    );
                } else {
    ...
    for (let i = 0; i < data.numCells; i++) {
        const gx = Math.min(
            GRID - 1,
            Math.max(0, Math.floor(((data.positions[i * 2] + 1) / 2) * GRID)),
        );
        const gy = Math.min(
            GRID - 1,
            Math.max(0, Math.floor(((data.positions[i * 2 + 1] + 1) / 2) * GRID)),
        );
    ...
    return {
        ...
        setPointRadius(r: number) {
            ...
            uniforms.paramsUniform.write(
                d.vec4f(
                    pointRadius,
                    (width * dpr) / Math.max(1, height * dpr),
                    selectionDimFactor,
                    adaptiveScale,
                ),
            );
