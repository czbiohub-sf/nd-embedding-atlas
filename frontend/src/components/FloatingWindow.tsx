...
export function FloatingWindow({ handle, title, children, className, extraTitleActions }: Props) {
    ...
    return (
        ...
        >
            ...
            >
                {title && (
                    <span className="flex-1 truncate font-medium text-[11px] text-muted-foreground/60">
                        {title}
                    </span>
                )}
                {extraTitleActions}
            ...
            <div
                ...
                style={
                    state.minimized ? { visibility: "hidden", pointerEvents: "none" } : undefined
                }
            >
                {children}
            </div>
            ...
            {!state.minimized && (
                <svg
                    viewBox="0 0 12 12"
                    className="pointer-events-none absolute right-1 bottom-1 z-10 size-3 text-white/20"
                >
