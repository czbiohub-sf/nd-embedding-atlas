  ...
  useEffect(() => {
    ...
    import("tweakpane" /* removed - will be replaced */ as any).then(({ Pane }) => {
      ...
      if (hasT) {
        ...
        pane
          .addBinding(tParamsRef.current, "T", {
      ...
      if (hasZ && viewMode === "2d") {
        pane
          .addBinding(zParamsRef.current, "Z", { min: 0, max: bounds.zMax ?? 0, step: 1 })
          .on("change", (ev: { value: number }) => actionsRef.current.setZIndex(Math.round(ev.value)));
      }

      // Crop size
      if (hasCellCoords) {
        pane
          .addBinding(bboxParamsRef.current, "bbox", { min: 50, max: 500, step: 10 })
          .on("change", (ev: { value: number }) => setCropSizeRef.current(Math.round(ev.value)));
      }
