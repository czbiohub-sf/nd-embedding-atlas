...
export function VolumeControls() {
  ...
  useEffect(() => {
    ...
    import("tweakpane" /* removed - will be replaced */ as any).then(({ Pane }) => {
      ...
      pane
        .addBinding(paramsRef.current, "opacity", { label: "opacity (log)", min: 0, max: 1, step: 0.01 })
      ...
      pane
        .addBinding(paramsRef.current, "step", { label: "step size", min: 0.25, max: 3, step: 0.25 })
        .on("change", (ev: { value: number }) => {
          for (const { layer } of layersRef.current) {
      ...
      pane
        .addBinding(paramsRef.current, "earlyStop", {
          label: "early stop α",
          min: 0.8,
          max: 1.0,
          step: 0.01,
