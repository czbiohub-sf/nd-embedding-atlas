...
import { useImageViewerServices } from "../context";
...
// Idetik doesn't export `Viewport`; recover the exact type from Idetik.addViewport.
...
function createRuntime(canvas: HTMLCanvasElement, mode: ViewMode): RuntimeResult {
  ...
  const camera = new OrthographicCamera({ left: 0, right: 1, bottom: 0, top: 1 });
...
export function ViewerProvider({ children, channelInstance = "docked" }: Props) {
  const { sharedState } = useImageViewerServices();
  // ── Mutable refs ──────────────────────────────────────────────────────
  ...
  const layerMapRef = useRef(new Map<string, { layer: Layer; callback: (state: LayerState) => void }>());
  ...
  // ── Size resync ───────────────────────────────────────────────────────
  // Idetik installs its own ResizeObserver, but @idetik/core ≤0.23.x loses
  ...
  // fix/pixel-size-observer-reconnect), own a ResizeObserver and drive the
  // same resync path: renderer buffer + per-viewport camera aspect.
  ...
  const observeCanvasSize = useCallback(
  ...
  const removeAllLayers = useCallback(() => {
    ...
    for (const [, entry] of layerMapRef.current) {
      entry.layer.removeStateChangeCallback(entry.callback);
      viewport.removeLayer(entry.layer);
    }
    layerMapRef.current.clear();
  }, []);
  ...
  useEffect(() => {
    if (channels.length > 0) {
      sharedState.publishChannels(channelInstance, channels);
    }
    return () => {
      sharedState.clearChannels(channelInstance);
    };
  }, [channels, channelInstance, sharedState]);

  // Publish the live Z plane so the gallery can fall back to it when the obs
  // dataframe has no per-obs `z` column.
  useEffect(() => {
    sharedState.publishZ(channelInstance, zIndex);
    return () => {
      sharedState.clearZ(channelInstance);
    };
  }, [zIndex, channelInstance, sharedState]);
