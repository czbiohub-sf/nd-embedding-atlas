...
export function createCompositor(
  ...
  //   - annotationBuffer is writable but not yet composed (no annotation tier)
  ...
  const compositorPipeline = root.createGuardedComputePipeline((x: number) => {
    ...
    const hasHi = (layerBits & LAYER_HIGHLIGHT) !== 0;
    const hasIso = (layerBits & LAYER_ISOLATION) !== 0;
    const hasSel = (layerBits & (LAYER_LASSO | LAYER_EXTERNAL)) !== 0;
  ...
  function dispatchIfDirty(encoder?: GPUCommandEncoder): void {
    ...
    if (encoder) {
      compositorPipeline.sizeUniform.write(d.vec3u(numPoints, 1, 1));
      compositorPipeline.pipeline.with(encoder).dispatchWorkgroups(Math.ceil(numPoints / 256));
