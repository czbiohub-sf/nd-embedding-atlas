...
import type { WrangleNodeHost, WrangleEditor } from "./contracts";
...
export function createWrangleBody(Editor: WrangleEditor) {
  return function WrangleBody({ host }: { host: WrangleNodeHost }) {
      ...
      <div className="flex h-full min-h-0 flex-col gap-1">
        ...
        <div className="flex shrink-0 items-center gap-1.5 px-0.5 font-mono text-3xs leading-none">
          {status === "error" ? (
            ...
            <span className="text-text-muted">prql · pass-through</span>
          ) : status === "reshaping" ? (
            <span
