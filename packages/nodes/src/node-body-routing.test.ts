...
import { countQuery } from "@ndea/nodes/count";
import { patchDatasetKey } from "@ndea/nodes/dataset";
import { classifyWrangleSql } from "@ndea/nodes/wrangle";
...
describe("node Body host routing", () => {
  ...
  test("Wrangle classification queries through the data capability", async () => {
    ...
    const malformed = await classifyWrangleSql(() => Promise.resolve([{ column_name: 42 }]), "SELECT 42");
