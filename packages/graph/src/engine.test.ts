...
describe("GraphEngine", () => {
  ...
  test("cache boundary: a clean upstream node is NOT recooked when only a dependent changes", () => {
    ...
    // Dirtying the source invalidates every dependent cache.
    engine.markDirty("src");
    engine.pull("sink");
    expect(src.calls()).toBe(2);
    expect(filter.calls()).toBe(3);
  });
  ...
  test("diamond dedup: a shared upstream cooks once per pull sweep", () => {
    ...
    engine.markDirty("src");
    engine.pull("sink");
    expect(src.calls()).toBe(2); // one additional cook in the next sweep
  });
  ...
  test("a stale sink disposer cannot unregister its replacement", () => {
    const engine = new GraphEngine({ schedule: (flush) => flush() });
    engine.addNode({ id: "sink", kind: "view", cook: () => "value" });
    const seen: Predicate[] = [];
    const listener = (value: Predicate) => seen.push(value);
    const stale = engine.registerSink("sink", listener);
    const active = engine.registerSink("sink", listener);

    stale();
    engine.markDirty("sink");
