...
If `ndea` is not found, confirm that the installer symlink directory is on `PATH`. Installed versions live under `~/.ndea/versions/`; the active `ndea` command is a symlink managed by update and rollback.

A `401` or `404` while installing from a restricted repository usually means the GitHub token is missing, expired, or lacks repository read access. Authenticate `gh`, then run the installer again.

If an update fails before the symlink swap, the current installation remains active. Use `ndea rollback` only when a previous version exists.
