...
Because pages are MDX, they can embed live React — video clips, and (planned) interactive node graphs users can play with inline. Build output goes to `docs/dist/` (gitignored); CI builds and deploys it to GitHub Pages on every push to `main` (see `.github/workflows/docs.yml`).
