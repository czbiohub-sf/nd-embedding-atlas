...
for (let i = 0; i < 30; i++) {
    ...
    if (body.status === "ready") {
        ...
        const scatter = await fetch(`http://localhost:5055/api/scatter-positions?embedding=X_phate&x_col=phate_0&y_col=phate_1`);
        console.log("scatter-positions →", scatter.status, scatter.headers.get("content-type"), "size:", (await scatter.arrayBuffer()).byteLength, "bytes");
        break;
    }
