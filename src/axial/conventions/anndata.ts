    ...
    async parse(group: any, storePath?: string): Promise<DataTree> {
        ...
        try {
            if (useParallel) {
                ...
                const obsGroup = await zarr.open(group.resolve("obs"), { kind: "group" });
                obs = await readDataFrame(obsGroup as any);
            }
        } catch (e) {
            throw new Error(
                `Failed to read obs DataFrame: ${e instanceof Error ? e.message : String(e)}`,
            );
        }
        ...
        try {
            if (useParallel) {
                ...
                const varGroup = await zarr.open(group.resolve("var"), { kind: "group" });
                varDf = await readDataFrame(varGroup as any);
            }
        } catch (e) {
            // var can have 0 columns (e.g. embedding-only stores) — warn but don't crash
            console.warn(
                `Warning: Failed to read var DataFrame: ${e instanceof Error ? e.message : String(e)}`,
            );
        }
        ...
        if (obs) {
            coords.push(
                new SimpleCoordArray(
                    "obs",
                    Array.isArray(obs.index) ? obs.index : Array.from(obs.index),
                    "string",
                    { source: "obs._index" },
                ),
            );
        ...
        if (varDf) {
            coords.push(
                new SimpleCoordArray(
                    "var",
                    Array.isArray(varDf.index) ? varDf.index : Array.from(varDf.index),
                    "string",
                    { source: "var._index" },
                ),
            );
        ...
        let X: any;
        try {
            const xLocation = group.resolve("X");
            try {
                ...
                try {
                    ...
                    const arrayMsg =
                        arrayErr instanceof Error ? arrayErr.message : String(arrayErr);
                    const isNotFound =
                    ...
                    if (!isNotFound) {
                        console.warn(
                            `Warning: X exists but failed to read — group error: ${groupMsg}, array error: ${arrayMsg}`,
                        );
                    }
                }
            }
