...
self.onmessage = async (event: MessageEvent<ReadColumnRequest>) => {
    ...
    try {
        ...
        const location = (group as any).resolve(colName);
        ...
        try {
            ...
            if (encoding === "categorical") {
                result = await readCategoricalWorker(colGroup as any);
            } else if (
                ...
                result = await readNullableWorker(colGroup as any);
            } else {
...
async function readArrayWorker(location: any): Promise<ColumnResult> {
    ...
    return {
        ...
        data: buffer,
        dtype: arr.dtype,
        length: view.byteLength / ((data as any).BYTES_PER_ELEMENT ?? 1),
    };
}

async function readCategoricalWorker(group: any): Promise<ColumnResult> {
    ...
    const codesBuffer = codesBuf.slice(
        codesView.byteOffset,
        codesView.byteOffset + codesView.byteLength,
    );
    ...
    try {
        ...
        const valsArr = await zarr.open((catGroup as any).resolve("values"), { kind: "array" });
...
async function readNullableWorker(group: any): Promise<ColumnResult> {
    ...
    if (maskData instanceof Uint8Array) {
        maskBuffer = (maskData.buffer as ArrayBuffer).slice(
            maskData.byteOffset,
            maskData.byteOffset + maskData.byteLength,
        );
    } else {
        // BoolArray → convert to Uint8Array
        const u8 = Uint8Array.from(maskData as Iterable<number>, (v: any) => (v ? 1 : 0));
