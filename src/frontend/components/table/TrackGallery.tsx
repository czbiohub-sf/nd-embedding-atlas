...
export function TrackGallery({ activeFrame, onFrameSelect, datasetKey }: TrackGalleryProps) {
        ...
        const unsub = cache.subscribe((event) => {
            if (
                ...
                if (typeof url === "string" && url.startsWith("blob:")) {
                    ...
                    console.error(
                        "[TrackGallery] crop query data is not a blob URL — possible leak",
                        typeof url,
                    );
                }
            }
    ...
    const colCount = Math.max(
        1,
        Math.floor((containerWidth + CARD_GAP) / (MIN_COL_WIDTH + CARD_GAP)),
    );
    const colWidth =
        containerWidth > 0
            ? (containerWidth - CARD_GAP * (colCount - 1)) / colCount
            : MIN_COL_WIDTH;
    ...
    useEffect(() => {
        ...
        for (let i = maxVisible + 1; i <= prefetchEnd; i++) {
            ...
            const cachedObs =
                frame.rowIndex != null
                    ? queryClient.getQueryData<{ x: number; y: number }>(
