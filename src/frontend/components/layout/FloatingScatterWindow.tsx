...
} from "../../stores/FloatingScatterStore";
...
function FloatingScatterItem({ entryId }: { entryId: string }) {
    ...
    const linkedState = useStore(panelStateStore, (s) =>
    ...
    const linkButton = (
        <Tooltip>
                            ...
                            setFloatingScatterLink(
                                ...
                                isLinked
                                    ...
                                    : Object.keys(
                                          ...
                                          Object.fromEntries(
                                              Array.from(panelStateStore.state.entries()).filter(
                                                  ([k]) => !k.startsWith("float-"),
                                              ),
                                          ),
                                      )[0],
                            )
            ...
            <TooltipContent side="bottom">
                {isLinked ? "Unlink from panel" : "Sync from panel"}
            </TooltipContent>
        </Tooltip>
    );
