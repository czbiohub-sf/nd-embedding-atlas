        ...
        (which: "min" | "max") => {
            ...
            function onMove(e: PointerEvent) {
                ...
                onRangeChangeRef.current?.(
                    toValRef.current(localMinRef.current),
                    toValRef.current(localMaxRef.current),
                );
            }
    ...
    return (
        <div className="absolute top-10 left-2 z-20 w-44 rounded-lg border border-white/[0.07] bg-card/80 p-2.5 backdrop-blur-md">
            ...
            <p className="mb-1.5 flex items-center gap-1 font-mono text-[10px] text-muted-foreground">
                ...
                {scaleBadge && (
                    <span className="shrink-0 text-[9px] text-accent-cyan">{scaleBadge}</span>
                )}
            </p>
            ...
            <ContextMenu>
                <ContextMenuTrigger>
                    <div
                        ref={barRef}
                        className="relative h-3 w-full cursor-context-menu select-none rounded-sm"
                    >
                        ...
                        {hasRange && (
                            <>
                                <div
                                    className="absolute inset-y-0 left-0 rounded-l-sm bg-black/50"
                                    style={{ width: minPct }}
                                />
                                <div
