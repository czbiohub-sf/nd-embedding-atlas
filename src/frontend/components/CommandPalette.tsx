...
export function CommandPalette({
    ...
    const canExport =
        exportData && exportData.filtered > 0 && exportData.filtered < exportData.total;
        ...
        <>
                                ...
                                >
                                    ...
                                    {exportData && (
                                        <span className="ml-auto text-[10px] text-muted-foreground/50 tabular-nums">
                                            {canExport
                                                ? exportData.filtered.toLocaleString()
                                                : "no selection"}
                                        </span>
                                    )}
                                </CommandItem>
                    ...
                    {page === "scatter" && (
                        <CommandGroup heading="Choose embedding">
                            {obsmEntries.map(([key]) => (
                                <CommandItem
                                    key={key}
                                    onSelect={() => dispatch(() => onAddScatter(key))}
                                >
                                    <LayoutGrid data-icon="inline-start" />
                                    {key.replace(/^X_/, "")}
                                </CommandItem>
            ...
            <Suspense fallback={null}>
                <ExportDialog
                    ...
                    filtered={exportData?.filtered ?? 0}
                />
            </Suspense>
        </>
    );
