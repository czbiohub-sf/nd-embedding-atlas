...
export const scatterKeys = {
    ...
    rangeIsolation: (col: string, vmin: number, vmax: number) =>
        ["scatter", "range-isolation", col, vmin, vmax] as const,
} as const;
...
export const trajectoryKeys = {
    track: (table: string, trackId: number, fovName: string) =>
        ["trajectory", table, trackId, fovName] as const,
} as const;
