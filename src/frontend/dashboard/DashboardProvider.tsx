...
function panelReducer(state: ChartPanelEntry[], action: PanelAction): ChartPanelEntry[] {
    switch (action.type) {
        ...
        case "REORDER_PANELS": {
            ...
            return action.ids
                .map((id) => byId.get(id))
                .filter((p): p is ChartPanelEntry => p != null);
        }
        default:
            return state;
    }
...
export function DashboardProvider({ children }: Props) {
    ...
    const addPanel = useCallback(
        (spec: ChartSpec) => dispatchPanels({ type: "ADD_PANEL", spec }),
        [],
    );
    const removePanel = useCallback(
        (id: string) => dispatchPanels({ type: "REMOVE_PANEL", id }),
        [],
    );
    const reorderPanels = useCallback(
        (ids: string[]) => dispatchPanels({ type: "REORDER_PANELS", ids }),
        [],
    );
    ...
    const meta = useMemo(
        () => ({ coordinator, brushSelection, table: TABLE }),
