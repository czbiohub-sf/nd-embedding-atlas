...
from ome_atlas.vz._duckdb import EmbeddingStore, mount_duckdb_endpoints
from ome_atlas.vz._prepare import _obsm_column_prefix, prepare_obs
...
def _resolve_frontend() -> str:
    ...
    2. **Bundled** -- ``importlib.resources`` looks for ``ome_atlas/_frontend/``
    ...
    ref = importlib.resources.files("ome_atlas") / "_frontend"
    if ref.is_dir():
        return str(ref)
    ...
    msg = (
        ...
        "  - Install from wheel: `uv pip install ome-atlas`"
    )
...
def create_app(
    ...
    *,
    ...
    async def get_metadata() -> dict:
        result: dict = {
            "version": importlib.metadata.version("ome-atlas"),
...
def serve(
    ...
    *,
    ...
    print(f"ome-atlas viewer: http://{host}:{port}")
