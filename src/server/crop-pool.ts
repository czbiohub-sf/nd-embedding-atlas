...
/** Resolve worker entrypoint URL — handles dev vs `bun build --compile` modes. */
function workerUrl(): string {
  // `bun build --compile` emits bundled worker entrypoints at
  // `/$bunfs/root/<name>.js`. Mirror readers.ts:300-303 exactly.
  const isCompiled = typeof Bun !== "undefined" && Array.isArray(Bun.embeddedFiles) && Bun.embeddedFiles.length > 0;
  return isCompiled ? "/$bunfs/root/server/crop-worker.js" : new URL("./crop-worker.ts", import.meta.url).href;
}
