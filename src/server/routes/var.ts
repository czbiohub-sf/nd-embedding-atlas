...
function varNamesOf(accessor: AnnDataAccessor): string[] {
    ...
    return Array.from(idx as Int32Array, (n) => String(n));
}
...
async function materialiseGeneColumn(
    state: ViewerState,
    gene: string,
    layer: string,
    colName: string,
    ...
    if (cursor !== nObs) {
        throw new Error(
            `Gene column row alignment mismatch: obs_base.nObs=${nObs} but accessors sum to ${cursor}`,
        );
    }
...
function findVarIndex(accessor: AnnDataAccessor, gene: string): number {
    ...
    for (let i = 0; i < idx.length; i++) {
        if ((idx as Int32Array)[i] === n) return i;
    }
    return -1;
}
...
async function loadGeneSlice(
    ...
    if ("data" in mat && "shape" in mat) {
        ...
        for (let i = 0; i < nObs; i++) out[i] = Number(d[i]);
        return out;
    }
    ...
    if (sparse.format === "csc") {
        ...
        for (let p = start; p < end; p++) {
            out[sparse.indices[p]] = Number(sparse.data[p]);
        }
    } else {
        ...
        for (let i = 0; i < nObs; i++) {
            ...
            if (re > rs) out[i] = Number(sparse.data[rs]);
        }
    }
    return out;
}
