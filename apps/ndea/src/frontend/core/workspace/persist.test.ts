...
describe("workspace v3 document schema", () => {
  test("serializes the canonical v3 runtime boundary", () => {
  ...
  test("strict validation rejects copied provenance and legacy editor fields", () => {
    const document = toPersistedDoc(emptyState()) as unknown as { version: 3; state: Record<string, unknown> };
    document.state.selection = null;
    expect(validateDoc(document, library).ok).toBe(false);

    const withNode = toPersistedDoc(emptyState()) as unknown as { version: 3; state: Record<string, unknown> };
...
describe("pure step migrations", () => {
  test("v1 first migrates coordination and then exact v3 identity", () => {
